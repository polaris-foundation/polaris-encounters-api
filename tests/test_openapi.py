...
from dhos_encounters_api.models.api_spec import dhos_encounter_api_spec
...
def test_openapi(tmp_path: str, app: Flask) -> None:
    ...
    generate_openapi_spec(dhos_encounter_api_spec, new_spec_path, api_blueprint)

    new_spec = yaml.safe_load(new_spec_path.read_bytes())
    existing_spec = (
        Path(__file__).parent.parent / "dhos_encounters_api/openapi/openapi.yaml"
    )
